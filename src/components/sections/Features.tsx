...
import { ProBentoCard, ProBentoGrid } from "@/components/ui/ProBentoGrid";
  ...
  return (
    <section id="features" dir={lang === "fa" ? "rtl" : "ltr"} className="border-b border-border/70 bg-background">
      <Container className="ds-section-padding">
        ...
        <ProBentoGrid className="feature-grid ds-grid ds-content-gap ds-motion-stagger grid gap-6 md:grid-cols-3">
          {content.cards.map((feature, index) => {
            const Icon = icons[index] ?? Sparkles;

            return (
              <SectionReveal key={feature.title} className="h-full">
                <ProBentoCard className="ds-subtle-ring ds-padding-card flex h-full flex-col">
