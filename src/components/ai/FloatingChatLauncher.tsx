...
export function FloatingChatLauncher({ lang }: FloatingChatLauncherProps) {
    ...
    const isRtl = lang === "fa";
    const copy = isRtl
        ? { label: "گفت‌وگو با دستیار آرندی", placeholder: "چه کمکی از دستیار آرندی برمی‌آید؟", submit: "ارسال پیام" }
        : { label: "Chat with Arandi Assistant", placeholder: "What would you like to know?", submit: "Send message" };
    ...
    return (
        ...
        >
            <form
                onSubmit={submit}
                className="ds-chat-shell ds-subtle-ring pointer-events-auto mx-auto flex w-full max-w-xl items-center gap-2 rounded-full bg-background/92 p-2 shadow-[0_22px_60px_-28px_rgba(15,23,42,0.55)] backdrop-blur-xl"
            >
                ...
                <button
                    type="submit"
                    disabled={!draft.trim()}
                    className="ds-focus-visible inline-flex size-10 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-[var(--elevation-1)] transition disabled:cursor-not-allowed disabled:opacity-45"
