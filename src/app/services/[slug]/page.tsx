...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
    ...
    const detail = await getPublicCollectionDetail("services", slug, lang).catch(() => null);
    return detail
        ? buildLocalizedMetadata({ path: `/services/${slug}`, lang, title: detail.title, description: detail.summary })
...
export default async function ServiceDetailPage({ params, searchParams }: Props) {
    const [{ slug }, lang] = await Promise.all([params, resolveLanguage(searchParams)]);
    const detail = await getPublicCollectionDetail("services", slug, lang).catch(() => notFound());
