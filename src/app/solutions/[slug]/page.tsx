...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
    ...
    const detail = await getPublicCollectionDetail("solutions", slug, lang).catch(() => null);
    return detail
        ? buildLocalizedMetadata({ path: `/solutions/${slug}`, lang, title: detail.title, description: detail.summary })
...
export default async function SolutionDetailPage({ params, searchParams }: Props) {
    const [{ slug }, lang] = await Promise.all([params, resolveLanguage(searchParams)]);
    const detail = await getPublicCollectionDetail("solutions", slug, lang).catch(() => notFound());
