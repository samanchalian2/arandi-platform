...
- Release `20260826T-brand-header-scale`, sourced from reviewed commit `cc8b724`, is active on `arandivps` for the canonical `https://arandi.io` hostname. DNS, HTTPS, HSTS, canonical redirects, CMS/Media migration, readiness, public-route checks, and FA/EN Browser QA are directly verified. The active Scrollwise header has a 48px brand symbol and a 16px desktop brand title; the prior VPS remains a rollback source and is not modified by this deployment.
...
1. Obtain stakeholder visual acceptance for the live Scrollwise release `20260826T-brand-header-scale` at canonical `https://arandi.io` (including the 48px/16px header brand scale and exact `#EDE9E1` footer), then observe GitHub CI for commits `e8a54c1`, `d8187cc`, and `cc8b724`. Preserve the active release and immediate rollback release on `arandivps`.
