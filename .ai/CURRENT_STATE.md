...
- Local validation passed: Prisma migration status (9 applied), 51 focused tests, strict typecheck, zero-warning lint, diff check, and the 63-route production build. Commit `cf4cd8e` was pushed to `origin/scrollwise` and deployed as VPS release `20260825T123724Z-scrollwise-footer` through the rollback-aware release script. Follow-up commit `98785ef` made the footer text white and restored the finale panel to a warm light surface; it is active as release `20260825T124756Z-scrollwise-light-surfaces`. Backup, Linux build, Prisma deployment (no pending migration), service activation, readiness, Home, Contact, Projects, Articles, Legal, and sitemap checks passed. Browser inspection of the active release confirmed white footer text, a light finale panel, and Persian RTL; 2.7GB disk remained after the second build.
